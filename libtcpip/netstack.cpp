...
namespace lwip {
    ...
    inline static void* netstack_tcp_linksocket(struct tcp_pcb* pcb, const std::shared_ptr<netstack_tcp_socket>& socket) {
        if (!pcb || !socket) {
            return NULL;
        }
    ...
    inline static std::shared_ptr<netstack_tcp_socket> netstack_tcp_getsocket(void* p) {
        if (!p) {
            return NULL;
        }
    ...
    inline static std::shared_ptr<netstack_tcp_socket> netstack_tcp_releasesocket(void* p) {
        if (!p) {
            return NULL;
        }
    ...
    inline static err_t netstack_tcp_closesocket(struct tcp_pcb* pcb);

    inline static bool netstack_tcp_closesocket(const std::shared_ptr<netstack_tcp_socket>& socket_);

    inline static err_t netstack_tcp_send(struct tcp_pcb* pcb, void* data, u16_t len, const std::function<void(struct tcp_pcb*)>& callback) {
        if (!pcb) {
            return ERR_ARG;
        }
    ...
    inline static void netstack_tcp_arg(struct tcp_pcb* pcb, void* arg) {
        if (pcb) {
            tcp_arg(pcb, arg);
        }
    }

    inline static void netstack_tcp_event(struct tcp_pcb* pcb, tcp_recv_fn recv, tcp_sent_fn sent, tcp_err_fn errf, tcp_poll_fn poll) {
        if (pcb) {
    ...
    inline static void netstack_tcp_closesocket(boost::asio::ip::tcp::socket& socket) {
        if (socket.is_open()) {
    ...
    inline static void netstack_tcp_closesocket(std::shared_ptr<boost::asio::ip::tcp::socket> socket) {
        if (socket) {
    ...
    inline static struct pbuf* netstack_pbuf_alloc(u16_t len) {
        if (!len) {
            return NULL;
        }
    ...
    inline static void netstack_pbuf_free(struct pbuf* buf) {
        if (buf) {
            pbuf_free(buf);
        }
    }

    inline static bool netstack_tunnel_send(const std::shared_ptr<netstack_tcp_socket>& socket_, void* data, int len, bool unsafe_);

    inline static err_t netstack_tcp_dorecv(void* arg, struct tcp_pcb* pcb, struct pbuf* p, err_t err) {
        LWIP_UNUSED_ARG(arg);
    ...
    inline static err_t netstack_tcp_dosent(void* arg, struct tcp_pcb* pcb, u16_t len) {
        LWIP_UNUSED_ARG(arg);
    ...
    inline static void netstack_tcp_doerrf(void* arg, err_t err) {
    ...
    inline static err_t netstack_tcp_dopoll(void* arg, struct tcp_pcb* pcb) {
        std::shared_ptr<netstack_tcp_socket> socket = netstack_tcp_getsocket(pcb->callback_arg);
        if (socket) {
    ...
    inline static bool netstack_socket_connect(const std::shared_ptr<netstack_tcp_socket>& socket_, const boost::asio::ip::tcp::endpoint& remoteEP_);
    inline static bool netstack_tunnel_open(const std::shared_ptr<netstack_tcp_socket>& socket_, boost::asio::ip::tcp::endpoint& remoteEP_);

    inline static err_t netstack_tcp_doaccept(void* arg, struct tcp_pcb* pcb, err_t err) {
        LWIP_UNUSED_ARG(arg);
        LWIP_UNUSED_ARG(err);
    ...
    inline static bool netstack_tunnel_dorecv(const std::shared_ptr<netstack_tcp_socket>& socket_) {
        if (!socket_) {
            return false;
        }
    ...
    inline static bool nestack_tunnel_post_all_unsent(const std::shared_ptr<netstack_tcp_socket>& socket_) {
        if (!socket_) {
            return false;
        }
    ...
    inline static bool netstack_socket_connect(const std::shared_ptr<netstack_tcp_socket>& socket_, const boost::asio::ip::tcp::endpoint& remoteEP_) {
        ...
        socket->async_connect(remoteEP_, [socket_](const boost::system::error_code& ec) {
            if (ec) {
                netstack_tcp_closesocket(socket_);
                return;
            }
    ...
    inline static bool netstack_tunnel_open(const std::shared_ptr<netstack_tcp_socket>& socket_, boost::asio::ip::tcp::endpoint& remoteEP_) {
        ...
        if (IP_IS_V4_VAL(socket_->remote_ip)) {
            uint32_t address_ = ip_addr_get_ip4_u32(&socket_->remote_ip);
            address_ = ntohl(address_);
            remoteEP_ = boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4(address_), socket_->remote_port);
        }
        else if (IP_IS_V6_VAL(socket_->remote_ip)) {
            boost::asio::ip::address_v6::bytes_type address_;
            memcpy(address_.data(), ip_2_ip6(&socket_->remote_ip), address_.size());
            remoteEP_ = boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v6(address_), socket_->remote_port);
        }
        else {
            return false;
        }
    ...
    inline static err_t netstack_ip_output(struct pbuf* p) {
        if (!p || !p->len) {
            return ERR_BUF;
        }
    ...
    inline static err_t netstack_ip_output_v4(struct netif* netif, struct pbuf* p, const ip4_addr_t* ipaddr) {
    ...
    inline static bool netstack_tcp_init() {
    ...
    inline static void netstack_check_timeouts() {
    ...
    bool netstack::input(const void* packet, int size) {
